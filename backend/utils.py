...
import pedalboard
import random
#from spec_utils import spectral_de_normalize_torch
import torch
...
def preprocess_wav(fpath_or_wav: Union[str, Path, np.ndarray],
                   source_sr: Optional[int] = None, isReverb: bool = False):
    """
    ...
    if isReverb:
        ...
        wav = reverb(wav, sample_rate=sample_rate)
...
def preprocess_wav_augmentation(fpath_or_wav: Union[str, Path, np.ndarray],
                   source_sr: Optional[int] = None, isReverb: bool = False, shft=24):
    # Load the wav from disk if needed
    if isinstance(fpath_or_wav, str) or isinstance(fpath_or_wav, Path):
        wav, source_sr = librosa.load(str(fpath_or_wav), sr=None)
    else:
    ...
    if random.randint(0, 9) < 3:
        s = (-1) * random.randint(1,shft)
        wav = librosa.effects.pitch_shift(wav, sr=sample_rate, n_steps=s)
    wav = normalize_volume(wav, audio_norm_target_dBFS, increase_only=True)
    #if webrtcvad:
    #    wav = trim_long_silences(wav)
...
def reconstruct_waveform(mel, n_iter=32):
    ...
    #denormalized = spectral_de_normalize_torch(temp).numpy()
