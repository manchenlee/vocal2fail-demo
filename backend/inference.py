...
encoder = Encoder(dim=32, in_channels=1, n_downsample=2).cuda()
G_trg= Generator(dim=32, out_channels=1, n_upsample=2, shared_block=ResidualBlock(features=shared_dim)).cuda()
...
def infer(S, multi):
    ...
    Z = Z + multi * torch.from_numpy(attr_vector_pp).cuda()
