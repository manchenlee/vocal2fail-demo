...
# load path
encoder.load_state_dict(torch.load("./models\\models\\encoder_99.pth", map_location=torch.device('cpu')))
G_trg.load_state_dict(torch.load("./models\\models\\G2_99.pth", map_location=torch.device('cpu')))
...
def infer(S, multi):
    ...
    mu, Z = encoder(X)
    attr_vector_pp = np.load('./models\\models\\attr.npy')
